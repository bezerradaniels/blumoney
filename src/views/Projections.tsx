    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div>
        <h2 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
          <TrendingUp className="w-6 h-6 text-emerald-600" /> Planejamento & Projeções (6 Meses)
        </h2>
        <p className="text-xs text-slate-500 mt-1">
          Analise os compromissos futuros acumulados em parcelamentos e faturas dos seus cartões.
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col justify-between">
            ...
            <div className="mt-4">
              <h2 className="text-3xl font-extrabold text-emerald-700 font-mono">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col justify-between">
            ...
            <div className="mt-4">
              <h2 className="text-3xl font-extrabold text-teal-700 font-mono">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4 shadow-xs">
          <div>
            ...
            <p className="text-xs text-slate-500">Detalhamento dos valores por cartão em cada mês futuro</p>
          </div>
          ...
          <div className="overflow-x-auto border border-slate-200 rounded-xl">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {cards.map((card) => (
                  <tr key={card.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    {projections.map((p) => (
                      <td key={p.monthKey} className="py-3 px-4 text-right font-mono text-slate-700">
              ...
              <tfoot className="bg-slate-50 font-bold border-t border-slate-200 text-xs">
                <tr>
                  <td className="py-3 px-4 text-slate-900 uppercase text-[10px]">Total Comprometido</td>
                  {projections.map((p) => (
                    <td key={p.monthKey} className="py-3 px-4 text-right font-mono text-emerald-800">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4 shadow-xs">
          <div>
            ...
            <p className="text-xs text-slate-500">Lançamentos futuros vinculados a compras parceladas</p>
          </div>

          <div className="space-y-3">
            {activeInstallmentGroups.length === 0 ? (
              <p className="text-xs text-slate-400 py-4 text-center">Nenhum parcelamento ativo no momento.</p>
            ) : (
                  ...
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-900 text-sm">{sampleTx.description.replace(/\s\(\d+\/\d+\)/, '')}</span>
                      ...
                      <p className="text-xs text-slate-500">
                        Cartão: <strong className="text-slate-800">{card?.name}</strong> • Categoria: {sampleTx.category}
                      </p>
                    </div>

                    <div className="text-right">
                      <span className="text-[10px] text-slate-500 block uppercase font-semibold">Valor Total Acumulado</span>
